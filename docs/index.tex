...
\caption{\label{fig:org2abc99c}Geometry of the BRB Benchmark}
\end{figure}

A picture of the parts of the BRB is shown in Fig. \ref{fig:org2abc99c} and this will be used to guide the modeling here.
Standard 5/16 bolt-nut-washers will be used for the assembly. Since Bolt thread interactions won't be modeled here, the bolt shank is modeled as a smooth cylinder (see \ref{org55c6a2b} for more details on bolt prestress modeling).
...
\item Fig \ref{fig:orgb9d3fea} shows a pictorial overview of the different options available in Abaqus for scripting support. The Abaqus PDE can be used for Abaqus python script development.
\end{itemize}
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figs/ovw.png}
\caption{\label{fig:orgb9d3fea}Options in Abaqus for Scripting}
\end{figure}

\subsection{Modeling Parts\hfill{}\textsc{Script:GUI}}
\label{sec:org34f1e0b}
...
This will be necessary for the application of bolt prestress (see \ref{org55c6a2b} below).
\begin{verbatim}
...
These two points will have to be on the same geometrical point but equal and opposite forces will be applied for the force application (see \ref{org55c6a2b} below for those steps).
