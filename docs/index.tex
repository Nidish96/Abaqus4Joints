...
\item Fig \ref{fig:orgbe5d006} shows a pictorial overview of the different options available in Abaqus for scripting support. The Abaqus PDE can be used for Abaqus python script development.
\end{itemize}
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figs/ovw.png}
\caption{\label{fig:orgbe5d006}Options in Abaqus for Scripting}
\end{figure}

\subsection{Modeling Parts\hfill{}\textsc{Script:GUI}}
\label{sec:org187dbde}
...
These two points will have to be on the same geometrical point but equal and opposite forces will be applied for the force application (see \ref{org17ce1da} below for those steps).
