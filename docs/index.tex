...
\caption{\label{fig:orgdb35edf}Geometry of the BRB Benchmark}
\end{figure}

A picture of the parts of the BRB is shown in Fig. \ref{fig:orgdb35edf} and this will be used to guide the modeling here.
Standard 5/16 bolt-nut-washers will be used for the assembly. Since Bolt thread interactions won't be modeled here, the bolt shank is modeled as a smooth cylinder (see \ref{org7bcd51d} for more details on bolt prestress modeling).
...
\item Fig \ref{fig:org13fd2d9} shows a pictorial overview of the different options available in ABAQUS for scripting support. The ABAQUS PDE can be used for ABAQUS python script development.
\end{itemize}
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{./figs/ovw.png}
\caption{\label{fig:org13fd2d9}Options in ABAQUS for Scripting}
\end{figure}

\subsection{Modeling Parts\hfill{}\textsc{Script:GUI}}
\label{sec:org8379cc2}
...
This will be necessary for the application of bolt prestress (see \ref{org7bcd51d} below).
\begin{verbatim}
...
These two points will have to be on the same geometrical point but equal and opposite forces will be applied for the force application (see \ref{org7bcd51d} below for those steps).
