...
\caption{\label{fig:orgbb785c3}Geometry of the BRB Benchmark}
\end{figure}

A picture of the parts of the BRB is shown in Fig. \ref{fig:orgbb785c3} and this will be used to guide the modeling here.
Standard 5/16 bolt-nut-washers will be used for the assembly. Since Bolt thread interactions won't be modeled here, the bolt shank is modeled as a smooth cylinder (see \ref{org064ea5f} for more details on bolt prestress modeling).
...
\item Fig \ref{fig:org7333692} shows a pictorial overview of the different options available in ABAQUS for scripting support. The ABAQUS PDE can be used for ABAQUS python script development.
\end{itemize}
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{./figs/ovw.png}
\caption{\label{fig:org7333692}Options in ABAQUS for Scripting}
\end{figure}

\subsection{Modeling Parts\hfill{}\textsc{Script:GUI}}
\label{sec:org0fb7b34}
...
This will be necessary for the application of bolt prestress (see \ref{org064ea5f} below).
\begin{verbatim}
...
These two points will have to be on the same geometrical point but equal and opposite forces will be applied for the force application (see \ref{org064ea5f} below for those steps).
