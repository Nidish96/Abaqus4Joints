...
\item Fig \ref{fig:orgbcc063a} shows a pictorial overview of the different options available in Abaqus for scripting support. The Abaqus PDE can be used for Abaqus python script development.
\end{itemize}
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figs/ovw.png}
\caption{\label{fig:orgbcc063a}Options in Abaqus for Scripting}
\end{figure}

\subsection{Modeling Parts\hfill{}\textsc{Script:GUI}}
\label{sec:org32bd273}
...
These two points will have to be on the same geometrical point but equal and opposite forces will be applied for the force application (see \ref{orge010a9c} below for those steps).
