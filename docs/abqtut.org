...
#+latex_header: \usepackage[margin=0.8in]{geometry} \usepackage{cleveref}
...
A three-bolted lap joint system known as the Brake-Reu{\ss} Beam (read more about the benchmark [[https://jointmechanics.org/index.php/Benchmarks#The_Brake-Reu%C3%9F_Beams][here]]) is used for the demonstrations.
...
1. *Preprocessing* ([[Cref:sec:prepro]]): discusses the CAD modeling ([[cref:sec:modeling]]) of the parts along with necessary set creation ([[cref:sec:setcreat]]). Of particular note here is the *realization of bolt prestress* (see more in [[cref:sec:boltprel]]), which is done in a manner that is most convenient for linear and nonlinear vibration simulations including prestress effects.
   ...
   _The model at the end of this step can be found in [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step0.cae][model\under{}step0.cae]]._
2. *Constraints* ([[Cref:sec:constr]]): discusses the relevant constraints for the tied interfaces (washers are fully tied, for instance), and the bolt preload realization ([[cref:sec:boltprel]]).
   ...
   _The model at the end of this step can be found in [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step1.cae][model\under{}step1.cae]]._
3. *Meshing* ([[Cref:sec:meshing]]): discusses the seeded meshing procedure. This section also includes instructions for an optional abaqus static prestress simulation ([[cref:sec:abqprs]]) that can be used to verify the correctness of the model so far.
   ...
   _The model at the end of this step can be found in [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step1a.cae][model\under{}step1a.cae]]. Additionally, [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step2.cae][model\under{}step2.cae]] is also available, which includes the above prestress test._
...
5. *Matrix Extraction* ([[Cref:sec:mex]]): discusses the format of the outputted matrix and provides a bash-python script to postprocess it ([[cref:sec:bscr]]). Additionally a MATLAB/Octave tutorial is provide ([[cref:sec:nlan]]) which conducts *nonlinear prestress analysis with sub-100 lines of code*.
   _The model at the end of everything can be found in [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step3.cae][model\under{}step3.cae]]._
6. *Relative Coordinates Pipeline* ([[Cref:sec:relcsp]]): discusses the pipeline that needs to be followe in ABAQUS for using relative coordinate DOFS. *This provides an ~2x reduction in model size without any approximations*, and is therefore recommended always.
...
The following presentation was delivered at the monthly seminar series of the [[https://jointmechanics.org/index.php/Main_Page][International Committee of Jointed Structures]] in March 2025.
It provides an overview of the tutorial. You can find the file in the repository [[https://github.com/Nidish96/Abaqus4Joints/blob/832076ec8b4464826727a101fc5bf3c435ec9701/icjm_pres/icjm_pres.pdf][here]].

#+html: <embed src="./figs/icjm_pres.pdf" width="80%" height="375">


#+latex: \pagebreak
* Preprocess
:PROPERTIES:
:CUSTOM_ID: sec:prepro
:END:
...
Standard 5/16 bolt-nut-washers will be used for the assembly. Since Bolt thread interactions won't be modeled here, the bolt shank is modeled as a smooth cylinder (see [[cref:sec:boltprel]] for more details on bolt prestress modeling).
...
3. Create a Reference Point Part called =REFPT=.
   This will be necessary for the application of bolt prestress (see [[cref:sec:boltprel]] below).
   #+begin_src python +n :tangle yes
...
4. Now import another instance of the reference point =REFPT= and translate it to be coincident with =BPT-1=.
   ...
   These two points will have to be on the same geometrical point but equal and opposite forces will be applied for the force application (see [[cref:sec:boltprel]] below for those steps).
...
The file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step1a.cae][model\under{}step1a.cae]] stores the CAE-file along with the mesh.
** (Optional, recommended) Verify correctness of model through frictionless prestress :GUI:
:PROPERTIES:
:CUSTOM_ID: sec:abqprs
:END:
...
4. It is often beneficial in this context to use *relative coordinates* at the interface (so we can get a 2x reduction in the total number of unknowns in the problem). 
   ...
   This is optional, but recommended - see instructions in [[cref:sec:relcsp]].
#+latex: \pagebreak   
* Relative Coordinates Pipeline
:PROPERTIES:
:CUSTOM_ID: sec:relcsp
:END:
In [[cref:sec:meshproc]] and [[cref:sec:mex]], the true nodal DOFS of the interfacial nodes are extracted for the HCBCMS.
Here, we instead extract the relative coordinates directly by introducing a set of *ghost nodes* at the interface whose DOFs are set to be equal to the difference between the DOFs of the top and bottom nodes.
We achieve this by creating Reference Point instances at the appropriate locations and constraining them using the *EQUATION* constraint.

Note that we will first insert the reference points into the model ([[cref:sec:insrpt]]) and the write the resulting model into an inp file.
*I have been unable to figure out a way to do this fully natively with the cae model object, since the node IDs of the reference point instances don't seem to be available unless we are just about to do the analysis.*
If someone can figure this out, please [[mailto:nidbid@gmail.com][write to me]]

We start with the file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step1a.cae][model\under{}step1a.cae]] for this.
** Insertion of Reference Point Instances                           :Script:
:PROPERTIES:
:CUSTOM_ID: sec:insrpt
:END:
...
3. Import the reference point (partname: =REFPT=) as instances into the assembly and move them to the nodal locations.
   ...
   #+end_src
4. Create a set of the reference points, then simplify the model, and save the resulting model to an inp file.
   The name of the resulting inp file will be =MeshedModel.inp=.
   #+begin_src python +n
     ...
     mdbm.close()
   #+end_src
** Process the inp-file model, renumber the bottom nodes, and conduct HCBCMS.
1. We first read in the inp file we saved in the above.
   #+begin_src python -n
     ...
     ras = mdl.rootAssembly
   #+end_src
2. Following a process similar to that in [[cref:sec:meshproc]], we renumber and align the bottom surface nodes to match the top surface nodes.
...
3. Create node sets for the top, bottom and relative coordinates.
   #+begin_src python +n
     ...
     ras.SetFromNodeLabels(name="RELCSET",
                           ...
                           unsorted=True)
   #+end_src
4. We now set equation constraints to set a meaning for the ghost nodes.
...
5. We next setup the "fixed interface" CMS by conducting a Frequency step while constraining all the DOFS of the =RELCSET= nodeset to be zero (interface stuck against each other).
   ...
   #+end_src
6. The substructuring step is finally setup.
   We request 6 more modes in =modeRange= since the "fixedInterface= model will have 6 Rigid Body Modes which don't add anything to model accuracy.
...
9. The script [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/readwritematvec.sh][readwritematvec.sh]] can be used to post process this mtx file just as before.    
** Nonlinear Prestress Analysis with MATLAB/OCTAVE
1. The same MATLAB/OCTAVE routines from [[cref:sec:nlan]] can be used here, with minor tweaks to account for the fact that the model is already in the relative coordinates frame.
...
2. Here are the results
   1. Nodal Displacements:
      #+ATTR_HTML: :width 600px
      #+ATTR_LATEX: :width \textwidth
      [[./figs/intdisps_rc.png]]
   2. Interfacial Pressures:
      #+ATTR_HTML: :width 600px
      #+ATTR_LATEX: :width \textwidth
      [[./figs/intpress_rc.png]]
3. The results can be seen to be almost nearly identical to those shown before.
4. Computationally the advantages are clear.
   The size of the model matrices with relative coordinates is 2078, while the size without is 4136.
   Using the relative DOF-coordinates, therefore, *one can achieve an almost 2x reduction in system size*, which is beneficial in every form of analysis.
#+latex: \pagebreak   
* Outro/Contact
:PROPERTIES:
:CUSTOM_ID: sec:outro
:END:
You can contact me at [[mailto:nidish@iitm.ac.in][nidish@iitm.ac.in]] or [[mailto:nidbid@gmail.com][nidbid@gmail.com]] for any questions/suggestions.
...
6. Files [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/d1_relcs.py][d1\under{}relcs.py]] and [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/e1_relcsnproc.py][e1\under{}relcsnproc.py]] provide replacements for files 4 & 5 above while using relative coordinates.
7. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/readwritematvec.sh][readwritematvec.sh]]: Contains the mtx reading script written with =bash= and =python=.
8. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/demo/main.m][main.m]]: Contains the MATLAB/OCTAVE file for doing the nonlinear static prestress analysis (also see [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/demo/RESFUN.m][RESFUN.m]] for the residue routine).
9. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/demo/main_rc.m][main\under{}rc.m]]: Contains the MATLAB/OCTAVE file for the nonlinear static prestress analysis using the relative coordinates representation. 
   
The [[https://github.com/Nidish96/Abaqus4Joints][Github repository]] has the above scripts as well as all the cae files.
** Details on this web-page documentation/offering suggestions
+ This website was generated using org-mode on emacs with the Bigblow html theme from
  [[https://github.com/fniessen/org-html-themes][https://github.com/fniessen/org-html-themes]].
  The collapsible details blocks from [[https://github.com/alhassy/org-special-block-extras][org-special-block-extras]] have also been used extensively.
+ The best way to offer suggestions would be to use the [[https://github.com/Nidish96/Abaqus4Joints/blob/main/docs/index.pdf][pdf version]] of the documentation, annotate it and [[mailto:nidish.balaji@ila.uni-stuttgart.de][mail it to me]].
  Alternately, you can also clone the [[https://github.com/Nidish96/Abaqus4Joints][repository]], make changes and create a pull request.
