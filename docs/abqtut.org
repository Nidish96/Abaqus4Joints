...
4. Now import the nuts, washers and bolts by importing the file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/c_nutwasherbolt_516.py]].
   You should be able to see the nut, washer and bolt, along with the half beam \& reference point created before, as follows after importing:
   #+ATTR_HTML: :width 600px
   [[./figs/nwb.png]]
   The Python script also assigns the material "STEEL" (see above) to the parts.

The file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step0.cae][model_step0.cae]] stores the CAE file at the end of the above steps (building parts, & partitioning).
*REGENERATE THIS AFTER THE PARTITIONED BOLT IS USED!*
...
4. Create two surfaces on the =NUT= model as shown in the figure below.
   ...
   *Note the direction axes carefully.*
   #+ATTR_HTML: :width 600px
   [[./figs/nutsurfs.png]]
5. Create two surface on the =WASHER= model and label them as =WSTOP= & =WSBOT= (short for Washer-Surface Top & Bottom).
...
1. Import the two beams and re-orient/move them as follows.
   *Note that the bolt-axis has to be pointed in the +z direction.*
   ...
   #+ATTR_HTML: :width 600px
   [[./figs/asb1.png]]
2. Import one instance each of the =BOLT= & =NUT= and 2 instances of the =WASHER= and assemble them as shown.
   ...
   Rename the washer instances in the top & bottom as =TOPWASHER-1= and =BOTWASHER-1= respectively.
   ...
   #+begin_src python +n
     ...
     for i in range(1, 4):
         ...
         mdl.Coupling(name='NPC-%d' %(i), controlPoint=ras.instances['NPT-%d' %(i)].sets['Set-1'],
                      ...
                      weightingMethod=UNIFORM)
   #+end_src
2. Now we constrain the X, Y, Rx, Ry, & Rz DOFs (all 5 DOFs other than the Z DOF, which is the bolt-axis) using equation constraints.
...
The file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step1.cae][model_step1.cae]] stores the CAE-file generated after the end of the above steps.
We will now proceed with meshing.
*CORRECT NW CONSTRAINTS AND REUPLOAD*
* Meshing
Since the model is reasonably symmetric, it is sufficient to mesh the model with a global seed, after seeding the interface area locally.
Switch to the =Mesh= module for this section.
** Meshing Instructions
...
The file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step1.cae][model_step1.cae]] stores the CAE-file along with the mesh.
*CORRECT NW CONSTRAINTS AND RE-UPLOAD*
** (Optional, recommended) Verify correctness of model through frictionless prestress
...
The file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step2.cae][model_step2.cae]] is the cae file containing the model with the above tests included.
* Postprocessing
...
6. We now simplify the model by removing all interaction properties and steps (except initial).
   ...
   #+begin_src python +n
     ...
     while len(tmp) > 1:
         del tmp[tmp.keys()[-1]]
   #+end_src
** Setup Fixed interface CMS (HCB-CMS) with linear frequency and substructure steps
...
5. Here is what the final HCBCMS step looks like in this case.
   ...
   #+begin_src fortran
     ...
     *End Step
   #+end_src   
** Run Job and Generate the mtx file.
...
** Postprocessing Exported Matrices
...
The file [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step3.cae][model_step3.cae]] is the final cae file that contains all of the above.
** Simple Analysis on MATLAB/OCTAVE
...
1. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/a_steelmat.py][a_steelmat.py]]: Loading material
3. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/b_halfbm.py][b_halfbm.py]]: Building the half-beam model
4. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/c_nutwasherbolt_516.py][c_nutwasherbolt_516.py]]: Building the bolt, nut, & washer models.
5. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/d_applyconstraints.py][d_applyconstraints.py]]: Applying the relevant constraints and introducing bolt prestress.
6. [[https://github.com/Nidish96/Abaqus4Joints/blob/main/scripts/e_nodeproc.py][e_nodeproc.py]]: The postprocessing script create surface node sets and setting up substructure analysis.
...
Don't forget to check out the repository in github at [[https://github.com/Nidish96/Abaqus4Joints/blob/main/assets/assembly/model_step3.cae][https://github.com/Nidish96/Abaqus4Joints]].
